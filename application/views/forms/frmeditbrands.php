<?=form_open_multipart($this->uri->uri_string(),array('class'=>'form-horizontal')); ?>
	<legend>Форма редактирования брендов</legend>
	<fieldset>
		...
		<div id="ProductTabContent" class="tab-content">
			<div class="tab-pane fade in active" id="main">
				...
				<div class="control-group">
					...
					<div class="controls">
						<textarea rows="10" class="span7 input-valid redactor" name="text"><?=$brand['text'];?></textarea>
						<span class="help-inline" style="display:none;">&nbsp;</span>
					</div>
				</div>
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" class="span1 digital" name="sort" value="<?=($brand['sort'] == 100000)? '':$brand['sort'];?>">
						<span class="help-inline" style="display:none;">&nbsp;</span>
					</div>
				</div>
			</div>
				...
				<table class="table table-striped table-bordered">
					...
					<?php for($i=0;$i<count($seasons);$i+=2):?>
						...
						<?php if(isset($seasons[$i]['id'])):?>
							...
							<td class="w10">
								<input type="checkbox" class="chSeason" name="seasons[]" <?=($seasons[$i]['checked'])?'checked="checked"':'';?> <?=($seasons[$i]['id'] == 0)?'disabled="disabled"':''?> value="<?=$seasons[$i]['id'];?>" />
							</td>
						<?php endif;?>
